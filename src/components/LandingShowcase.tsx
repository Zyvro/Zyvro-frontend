...
import type { WorkflowCard } from "@/lib/api"
import { usePublicWorkflows } from "@/lib/hooks"
...
// public listing reads without an account, so a first-time visitor sees real
// work rather than a mockup.

const MAX_TILES = 6

function Tile({ workflow }: { workflow: WorkflowCard }) {
  return (
    <Link
      href={`/w/${workflow.id}`}
    ...
    >
      <WorkflowThumb preview={workflow.preview} className="h-44" />
      <div className="flex flex-1 flex-col gap-1 border-t border-white/[0.06] p-4">
        ...
        {workflow.description && (
          <p className="line-clamp-2 text-xs leading-relaxed text-muted-foreground">{workflow.description}</p>
        )}
        <div className="mt-auto flex items-center justify-between gap-2 pt-2 text-[11px] text-muted-foreground">
          <span className="truncate">{workflow.author_name}</span>
          <span className="shrink-0">
            {workflow.node_count} nodes · {workflow.edge_count} links
          </span>
        </div>
      </div>
    </Link>
...
export function LandingShowcase({ className }: { className?: string }) {
  const { data, isLoading, isError } = usePublicWorkflows()
  // A workflow that has never run has no thumbnail to show, so it is left out
  // rather than rendered as an empty frame.
  const items = (data ?? []).filter((wf) => hasPreview(wf.preview)).slice(0, MAX_TILES)
    ...
    <section className={cn("mx-auto max-w-6xl px-6", className)}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading
          ...
          : items.map((wf) => <Tile key={wf.id} workflow={wf} />)}
      </div>
    </section>
  )
}
